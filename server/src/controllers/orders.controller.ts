...
import { sendPurchaseConfirmation } from '../services/email.service';
...
export const createOrder = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (billingCycle === '1_year' && plan.price1Year) baseAmount = plan.price1Year * 12;
    else if (billingCycle === '2_years' && plan.price2Year) baseAmount = plan.price2Year * 24;
  ...
  try {
    ...
    if (status === 'success') {
      ...
      if (user) {
        ...
        sendPurchaseConfirmation(user.email, user.name, order.plan.name, order.amount).catch(console.error);
      }
    }
